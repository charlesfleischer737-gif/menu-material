...
export function OfferPreview({
  ...
  }, [draft, restaurant, dishes, format]);
  return (
    <div
...
export default function PromotionWorkspace({
  ...
  const persist = useCallback(async (): Promise<Row> => {
    ...
    if (comparable(draft) === comparable(previous.draft)) return previous;
    const promise = api("promotions/" + previous.id, {
      ...
      try {
        if (comparable(current.current) === comparable(draft))
          sessionStorage.removeItem(recoveryKey(previous.id));
        else
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (current.current) void persist().catch(() => {});
    }, 10000);
  ...
  const open = async (p: Row) => {
    ...
    setAccurate(false);
    setStatus(restored ? "Restoring unsaved edits…" : "Saved");
    setError("");
  ...
  useEffect(() => {
    ...
    if (p) void Promise.resolve().then(() => open(p));
  ...
  return (
    <section className="promotion-workspace">
      <div className="promotion-toolbar">
        <div>
          <h2>Promotions</h2>
          <p className="muted">One dish. Everything you need to share it.</p>
        </div>
        <Button onClick={() => action("Starting special", () => create())}>
          <Plus /> Promote tonight’s special
        </Button>
      </div>
      <div className="promotion-shelf">
        {(state.promotions || []).map((p: Row) => (
          <button
            key={p.id}
            onClick={() => action("Opening promotion", () => open(p))}
            className={selected?.id === p.id ? "selected" : ""}
          >
            <strong>{p.draft.title || "Untitled special"}</strong>
            <span>
              {p.status}
        ...
        <>
          ...
          <div className="promotion-grid">
            <div className="promotion-editor">
              <div className="panel">
                <p className="eyebrow">01 · YOUR OFFER</p>
                ...
                {newDish && (
                  <div className="inline-dish">
                    ...
                    <div className="button-row">
                      ...
                      <Button variant="ghost" onClick={() => setNewDish(null)}>
                        Cancel
                      </Button>
                    </div>
                  </div>
              ...
              {selectedDish && (
                <div className="panel">
                  <p className="eyebrow">02 · YOUR FOOD PHOTO</p>
                  <label className="field">
                  ...
                  <p className="fine">
                    ...
                    whole plate, shoot near a window and keep the phone steady.
                  </p>
            ...
            <div className="promotion-preview-column">
              <div className="panel preview-panel">
                <p className="eyebrow">03 · REVIEW YOUR PACKAGE</p>
                <Tabs
                  value={format}
                ...
                >
                  <TabsList className="export-tabs">
                    ...
                    <TabsTrigger value="clean">Menu image</TabsTrigger>
                  </TabsList>
                </Tabs>
                <OfferPreview
                  draft={form}
                ...
                />
                <details className="dish-options">
                  <summary>Crop & destination</summary>
                  <label className="field">
                    Download for
                    <select
                      value={format}
                    ...
                    >
                      {Object.entries(exportFormats).map(([v, x]) => (
                        <option value={v} key={v}>
                          {x.label}
                        </option>
                      ))}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      action("Preparing download", async () => {
                        ...
                        setStatus("Downloaded · not published by download");
                      })
                    }
                  >
                    <Download /> Download{" "}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        action("Marking sold out", async () => {
                          ...
                          await open(
                            (await api("promotions/" + selected.id)).promotion,
                          );
                          await refresh();
                        })
                      }
