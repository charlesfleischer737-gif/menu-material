...
Start in **Photo Studio**, **Menu Builder**, or **Post Maker**, with **My Dishes** as the shared library. New accounts start with a guided photo upload, three recommended styles and a direct Create action. Photo Studio keeps its 28-style catalog, fine-tuning, framing and immediate quick edits available on demand. The overview resumes saved work, and Post Maker offers ten Instagram designs across four stages. Earlier promotion campaigns and pilot tools remain under **More tools**. See [the core experience guide](docs/CORE_EXPERIENCE.md) for the flow, validation and connection status, and [the expansion guide](docs/PROMOTION_EXPANSION.md) for retained pilot tools.
