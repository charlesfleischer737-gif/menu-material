...
- **Photo Studio:** a single-page style → upload → create workbench, with camera/drop zone, original retention and normalized working image, optional name, no-photo illustration path, 56 visual presets in seven browsable collections, optional framing/lighting/surface/plate/angle/reference controls, one explicit result, before/after/history, fidelity approval, quick local adjustments saved as separate versions, restaurant look saving, and reuse into menus, posts and clean delivery exports.
...
New accounts without saved work land directly in Photo Studio. The style-first workbench keeps the full 56-style library and seven category filters beside an upload, settings and Create panel. Filtering categories does not select a style. Uploading or replacing a photo preserves the chosen style and controls. Existing pre-create drafts (internal steps 1–3) all resume in this single surface. Results and export stages retain their saved positions, with explicit links back to the style library and the prior result.

Optional vision analysis uses the normalized upload and returns the visible subject, food family, drink subtype and confidence. Suggestions are shown only for a high-confidence, single-subject photo (or the owner’s manual classification), and are bound to that upload ID. Beer, spirits, cocktails, coffee, tea and other beverages get drink examples, even when the output format is for delivery. No generic recommendations appear while analysis is pending, unavailable or uncertain. Analysis never changes the selected style. JSON response requests now include the required JSON instruction in the user input as well as the main instructions; live testing caught the previous request rejection. A manual correction takes priority over a late analysis result, and stale responses for replaced photos are ignored. Creation can proceed while analysis runs.

Desktop layouts use a scrollable image gallery and a persistent creation panel; mobile layouts keep a compact category rail and direct access to upload/create. Advanced photographic controls, framing, source replacement, camera capture, description-only illustrations, style references, saved restaurant looks and free quick edits remain available.

The waiting screen shows the saved original and selected style, an explicitly estimated progress bar, and three estimated steps for lighting, setting and detail. The real queue state remains distinct. Progress never reaches 100% from a timer; only an actual returned image leaves the loading screen. Longer jobs receive a calm “still creating” message instead of a stalled countdown. Reduced-motion preferences are respected. Result review opens with a draggable, keyboard-accessible comparison of the full original and edited images. Individual original/result views and zoom remain available. After naming and fidelity approval, “Save & download” saves the image and starts its download in one action; delivery exports still require the full-dish crop check. Quick adjustments must be saved as a new version first. If a download fails after approval, the approved photo remains in My Dishes and the download can be retried.
