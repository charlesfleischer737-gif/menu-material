...
- **Photo Studio:** a single-page style → upload → create workbench, with camera/drop zone, original retention and normalized working image, optional name, no-photo illustration path, 28 visual presets in seven browsable collections, optional framing/lighting/surface/plate/angle/reference controls, one explicit result, before/after/history, fidelity approval, quick local adjustments saved as separate versions, restaurant look saving, and reuse into menus, posts and clean delivery exports.
